  ...
  var sweetAlertConfirm = function(event) {
    ...
    if (sweetAlertConfirmConfig.confirmButtonColor !== null) {
      ...
      function afterAlertCallback(r){
        if (!r && swalOptions.onCancelTitle && swalOptions.onCancelText) {
          setTimeout(function(){
            var cancelConfirmOptions = {title: swalOptions.onCancelTitle, text: swalOptions.onCancelText, type: "error"};
            swal(cancelConfirmOptions);
          }, 200);
        }
      ...
      swal(swalOptions, function(e) { afterAlertCallback(e); });
